# This Makefile is for the Test::System extension to perl.
#
...
#     ABSTRACT => q[Test suite oriented for testing system administration tasks]
#     AUTHOR => q[Pablo Fischer (pablo@pablo.com.mx).]
#     DIR => []
#     DISTNAME => q[Test-System]
#     NAME => q[Test::System]
#     NO_META => q[1]
#     PL_FILES => {  }
#     PREREQ_PM => { TAP::Formatter::Console=>q[3.17], ExtUtils::MakeMaker=>q[6.42], Class::Factory=>q[1.05], YAML::Syck=>q[1.07], TAP::Formatter::HTML=>q[0.08], TAP::Harness=>q[3.17], Test::More=>q[0.94], File::Basename=>q[2.74], Moose=>q[0.87] }
#     VERSION => q[0.12]
...
PM_TO_BLIB = lib/Test/System/Helper.pm \
...
DISTNAME = Test-System
DISTVNAME = Test-System-0.12
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Test/System/Helper.pm $(INST_MAN3DIR)/Test::System::Helper.$(MAN3EXT) \
	  lib/Test/System/Output/Factory.pm $(INST_MAN3DIR)/Test::System::Output::Factory.$(MAN3EXT) \
	  lib/Test/System.pm $(INST_MAN3DIR)/Test::System.$(MAN3EXT) 
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Test suite oriented for testing system administration tasks</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Pablo Fischer (pablo@pablo.com.mx).</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Class-Factory" VERSION="1,05,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="ExtUtils-MakeMaker" VERSION="6,42,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-Basename" VERSION="2,74,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Moose" VERSION="0,87,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="TAP-Formatter-Console" VERSION="3,17,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="TAP-Formatter-HTML" VERSION="0,08,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="TAP-Harness" VERSION="3,17,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Test-More" VERSION="0,94,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="YAML-Syck" VERSION="1,07,0,0" />' >> $(DISTNAME).ppd
